...
from models.losses import get_alpha, softmax_helper, FocalLoss, mIoULoss, mmIoULoss, BCELoss
import torch
...
class CDTrainer():
    ...
    def _backward_G(self):
        ...
        if self.multi_scale_train == True:
            ...
            self.G_loss = self._pxl_loss(self.G_pred[-1], gt) + 0.5 * torch.nn.MSELoss()(self.G_pred[-3] , self.G_pred[-2]) + 0.5 * torch.nn.MSELoss()(self.G_pred[-5] , self.G_pred[-4])
    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            for self.batch_id, batch in tqdm(enumerate(self.dataloaders['train'], 0), total=total):
                ...
                self._backward_G()
                # self.optimizer_G.param_groups[0]['capturable'] = True
